...
interface ContactDAO {
    ...
    @Query("SELECT contact.id, contact.firstname, contact.lastname,  (\n" +
            "SELECT location.value  FROM location WHERE location.contactId =  contact.id AND location.week = :week AND location.day = :day\n" +
            ") as locationValue  FROM contact\n")
    fun getAllContactListInfo(day: Int, week: Int): LiveData<List<ContactListInfo>>
